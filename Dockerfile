...
ARG BUILD_CERTBOT
ARG BUILD_HOSTNAME
ARG BUILD_ISPCONFIG_VERSION
ARG BUILD_ISPCONFIG_DROP_EXISTING
ARG BUILD_ISPCONFIG_MYSQL_DATABASE
ARG BUILD_ISPCONFIG_PORT
ARG BUILD_ISPCONFIG_USE_SSL
ENV BUILD_JAILKIT_VERSION="2.21"
ARG BUILD_LOCALE
ARG BUILD_MYSQL_HOST
ARG BUILD_MYSQL_PW
ARG BUILD_MYSQL_REMOTE_ACCESS_HOST
ARG BUILD_PHPMYADMIN
ARG BUILD_PHPMYADMIN_PW
ARG BUILD_PHPMYADMIN_USER
ARG BUILD_PHPMYADMIN_VERSION
ARG BUILD_PRINTING
ARG BUILD_REDIS
ARG BUILD_ROUNDCUBE_VERSION
ARG BUILD_ROUNDCUBE_DB
ARG BUILD_ROUNDCUBE_DIR
ARG BUILD_ROUNDCUBE_PW
ARG BUILD_ROUNDCUBE_USER
ARG BUILD_TZ
...
RUN apt-get -y update && \
    ...
    locale-gen && \
    export LANG="${BUILD_LOCALE}.UTF-8" && \
    export LANGUAGE="${BUILD_LOCALE}:en" && \
    export LC_ALL="${BUILD_LOCALE}.UTF-8" && \
...
RUN touch "/etc/mailname"; \
    ...
    USER_EXISTS=$(printf "SELECT EXISTS(SELECT * FROM mysql.user WHERE User = '%s')" "${BUILD_ISPCONFIG_MYSQL_USER}" | mysql --skip-column-names -h "${BUILD_MYSQL_HOST}" -uroot -p"${BUILD_MYSQL_PW}" || true); \
