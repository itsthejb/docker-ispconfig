...
RUN apt-get -qq update && \
    apt-get -qq --no-install-recommends install apt-utils locales && \
...
# --- 1 Preliminary
    apt-get -qq --no-install-recommends install cron patch rsyslog rsyslog-relp logrotate supervisor git sendemail rsnapshot wget sudo; \
...
# --- 2 Install the SSH server
    apt-get -qq --no-install-recommends install ssh openssh-server; \
# --- 3 Install a shell text editor
    apt-get -qq --no-install-recommends install nano vim-nox; \
# --- 6 Change The Default Shell
    printf "dash  dash/sh boolean no\n" | debconf-set-selections; \
    dpkg-reconfigure dash; \
# --- 7 Synchronize the System Clock
    apt-get -qq --no-install-recommends install ntp ntpdate; \
# --- 8a Install MySQL (optional)
    apt-get -qq --no-install-recommends install mariadb-client; \
...
# --- 8b Install Postfix, Dovecot, and Binutils
    apt-get -qq update && apt-get -qq --no-install-recommends install postfix postfix-mysql postfix-doc getmail4 rkhunter binutils dovecot-imapd dovecot-pop3d dovecot-mysql dovecot-sieve dovecot-lmtpd libsasl2-modules; \
    apt-get clean && rm -rf /var/lib/apt/lists/*
...
# --- 9 Install SpamAssassin, and ClamAV
    (crontab -l; printf "\n") | sort - | uniq - | crontab -; \
    apt-get -qq update && apt-get -qq --no-install-recommends install spamassassin clamav sa-compile clamav-daemon unzip bzip2 arj nomarch lzop gnupg2 cabextract p7zip p7zip-full unrar-free lrzip apt-listchanges libnet-ldap-perl libauthen-sasl-perl clamav-docs daemon libio-string-perl libio-socket-ssl-perl libnet-ident-perl zip libnet-dns-perl libdbd-mysql-perl postgrey; \
    apt-get clean && rm -rf /var/lib/apt/lists/*
...
# --- 10 Install Apache Web Server and PHP
    if [ ${BUILD_MYSQL_HOST} = "localhost" ]; then service mysql restart; fi; \
    apt-get -qq update && apt-get -qq --no-install-recommends install apache2 apache2-doc apache2-utils libapache2-mod-php php7.3 php7.3-common php7.3-gd php7.3-mysql php7.3-imap php7.3-cli php7.3-cgi libapache2-mod-fcgid apache2-suexec-pristine php-pear mcrypt  imagemagick libruby libapache2-mod-python php7.3-curl php7.3-intl php7.3-pspell php7.3-recode php7.3-sqlite3 php7.3-tidy php7.3-xmlrpc php7.3-xsl memcached php-memcache php-imagick php-gettext php7.3-zip php7.3-mbstring memcached libapache2-mod-passenger php7.3-soap php7.3-fpm php7.3-opcache php-apcu; \
    apt-get clean && rm -rf /var/lib/apt/lists/*; \
...
# --- 11 Free SSL RUN mkdir /opt/certbot
    if [ ${BUILD_CERTBOT} = "yes" ]; then apt-get -qq --no-install-recommends install certbot; fi; \
# --- PHP-FPM
...
# --- 12 Install Mailman (Doesn't really work (yet))
    ...
    apt-get -qq --no-install-recommends install mailman; \
    apt-get clean && rm -rf /var/lib/apt/lists/*
...
# --- 13 Install PureFTPd
    apt-get -qq update && apt-get -qq --no-install-recommends install pure-ftpd-common pure-ftpd-mysql; \
    apt-get clean && rm -rf /var/lib/apt/lists/*; \
...
RUN apt-get -qq update && apt-get -qq --no-install-recommends install lsb-release unbound dnsutils haveged; \
    printf "do-ip6: no\n" > /etc/unbound/unbound.conf.d/no-ip6v.conf; \
    if [ $BUILD_REDIS = "yes" ]; then \
        apt-get -qq --no-install-recommends install redis-server; \
    ...
    apt-get -qq update && apt-get -qq --no-install-recommends install rspamd; \
...
# --- 15 Install Webalizer and AWStats
    apt-get -qq --no-install-recommends install webalizer awstats geoip-database libclass-dbi-mysql-perl libtimedate-perl; \
    apt-get clean && rm -rf /var/lib/apt/lists/*
COPY ./build/etc/cron.d/awstats /etc/cron.d/

# --- 16 Install Jailkit
# install package building helpers
RUN apt-get -qq update && apt-get -qq --no-install-recommends install build-essential autoconf automake libtool flex bison debhelper binutils; \
...
# --- 17 Install fail2ban and UFW Firewall
    ...
    apt-get -qq update && apt-get -qq --no-install-recommends install fail2ban ufw; \
    apt-get clean && rm -rf /var/lib/apt/lists/*;
...
RUN service fail2ban restart; \
    ...
    if [ ${BUILD_PHPMYADMIN} = "yes" ]; then \
        if [ ${BUILD_MYSQL_HOST} = "localhost" ]; then \
            ...
            apt-get -qq --no-install-recommends install php-imagick php-phpseclib php-php-gettext php7.3-common php7.3-mysql php7.3-gd php7.3-imap php7.3-json php7.3-curl php7.3-zip php7.3-xml php7.3-mbstring php7.3-bz2 php7.3-intl php7.3-gmp ;\
...
# --- 23 Install printing stuff
    if [ $BUILD_PRINTING = "yes" ]; then \
        apt-get -qq update; \
        apt-get -qq --no-install-recommends install --fix-missing -y libdmtx-utils dblatex latex-make cups-client lpr; \
        apt-get clean && rm -rf /var/lib/apt/lists/*; \
    fi; \
#
# docker-extensions
